...
export const rmspace = (st:string)=>{
  st = st.replace(/\d/g, ' ');
    [',','.',';','!','?','-'].forEach(e=>{
      if(st.includes(e)){
        st = st.replace(e,"")
...
export const hoverHandeler = (n:string)=>{
...
export const hoverHandelerrm = (n:string)=>{
  let tl = gsap.timeline()
  console.log(n)
  tl.to(`.${n}-b`,{
    x:"0%",
...
export default function Footer() {
  ...
  return (
    <div className='flex lg:flex-row flex-col justify-between bg-zinc-100 h-screen lg:p-0 p-8'>
      <div className='lg:p-20 pt-32 flex flex-col w-1/2 justify-between h-full'>
        ...
        <p className='lg:text-[12rem] text-[5rem] leading-[0.7] footerdiv'>EYE-</p>
        <p className='lg:text-[12rem] text-[5rem] leading-[0.7] footerdiv'>OPENING</p>
        </div>
        <div>
        </div>
      </div>
      <div className='lg:px-20' ref={linkcontainer}>
        <p className='footerdiv lg:text-[11rem] text-[5rem]'>PRESENTATIONS</p>
              ...
              <ul>
                {
                  ['202-1965 W 4th Ave','Vancouver, Canada','30 Chukarina St','Lviv, Ukraine'].map((e,i)=>(
                    <li key={i} className={`link cursor-pointer overflow-hidden relative ${i==1 ?'pb-5':''}`} onMouseLeave={()=>{hoverHandelerrm(rmspace(e))}} onMouseOver={()=>{hoverHandeler(rmspace(e))}}>
                      <div className={`${rmspace(e)}-b link-b n-1`}></div>
                      <div className={`${rmspace(e)}-a link-a`}></div>
                      {e}
                    </li>
                  ))
                }
              ...
              <ul>
                {
                  ['Home','Services','Our work','About us','Insights','Contact us'].map((e,i)=>(
                    <li key={i} className='link cursor-pointer overflow-hidden relative' onMouseLeave={()=>{hoverHandelerrm(rmspace(e))}} onMouseOver={()=>{hoverHandeler(rmspace(e))}}>
                      <div className={`${rmspace(e)}-b link-b`}></div>
                      <div className={`${rmspace(e)}-a link-a`}></div>
                      {e}
                    </li>
                  ))
